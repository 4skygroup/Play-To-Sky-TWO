      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-wrap gap-4 mt-2">
            {(
              t("genesis.stats", { returnObjects: true }) as {
                value: string;
                label: string;
              }[]
            ).map((stat, index) => (
              <div
                key={index}
                className="bg-white/5 border border-white/10 rounded-2xl px-6 py-5 flex-1 min-w-[140px]"
              >
                <div className="font-glacial font-bold text-t1 text-white">
                  {stat.value}
                </div>
                <div className="font-glacial text-t7 text-white/50 uppercase tracking-wider mt-1">
                  {stat.label}
                </div>
              </div>
            ))}
